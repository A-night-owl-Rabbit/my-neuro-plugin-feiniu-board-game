(function () {
  ...
  function renderXiangqiTrack(s) {
    const lm = s.lastMove;
    if (!lm || typeof lm.fr !== 'number' || typeof lm.to !== 'number') return '';
    const cols = 9, rows = 10;
    const fc = ((lm.fr % cols) / (cols - 1)) * 100;
    const fr = (Math.floor(lm.fr / cols) / (rows - 1)) * 100;
    const tc = ((lm.to % cols) / (cols - 1)) * 100;
    const tr = (Math.floor(lm.to / cols) / (rows - 1)) * 100;
    const sideClass = lm.player === -1 ? 'xq-track--black' : 'xq-track--red';
    return `<svg class="xq-track ${sideClass}" preserveAspectRatio="none" viewBox="0 0 100 100"><line x1="${fc}" y1="${fr}" x2="${tc}" y2="${tr}"/></svg>`;
  }
  function renderXiangqi(s, ended = false) {
    ...
    const chipText = ended ? '已终局' : (s.aiThinking ? '🤔 肥牛思考中…' : '对弈中...');
    const lastFr = s.lastMove && typeof s.lastMove.fr === 'number' ? s.lastMove.fr : -1;
    const lastTo = s.lastMove && typeof s.lastMove.to === 'number' ? s.lastMove.to : -1;
    const lastSide = s.lastMove?.player === -1 ? 'is-last-black' : 'is-last-red';
    const track = renderXiangqiTrack(s);
    if (intersection) {
      const pieces = []; const columns = 9; const rows = 10;
      for (let r = 0; r < rows; r++) for (let c = 0; c < columns; c++) {
        const idx = r * columns + c; const value = board[idx] || 0; const selected = s.sel === idx; const isHint = hints.has(idx);
        const isLastTo = idx === lastTo; const isLastFrom = idx === lastFr;
        const lastCls = isLastTo ? `is-last-to ${lastSide}` : isLastFrom ? `is-last-from ${lastSide}` : '';
        pieces.push(`<button class="xq-point ${selected ? 'is-selected' : ''} ${value ? 'has-piece' : ''} ${isHint ? 'is-hint' : ''} ${lastCls}" data-i="${idx}" style="left:${columns === 1 ? 50 : (c / (columns - 1)) * 100}%; top:${rows === 1 ? 50 : (r / (rows - 1)) * 100}%" ${locked ? 'disabled' : ''}>${icons.xiangqiPieceHtml(value)}</button>`);
      }
    ...
    return `<div class="board-head"><div><h2>中国象棋 🏮</h2><p>交叉点落子，红方先行哦~</p></div><div class="chip-row"><span class="chip">${chipText}</span></div></div><div class="xiangqi-board xiangqi-board--intersection ${ended ? 'is-ended' : ''} ${s.aiThinking ? 'is-thinking' : ''}"><div class="board-wood board-wood--xiangqi"><div class="board-pad"><div class="board-lines">${vertical}${horizontal}</div><div class="board-overlays"><span class="xq-river">楚河 汉界</span>${diagonals}${marks.map((cls) => `<span class="${cls}"></span>`).join('')}</div>${track}<div class="xq-pieces">${pieces.join('')}</div></div></div></div><p class="board-hint">${s.aiThinking ? '⏳ 肥牛在思考最佳应对，棋盘暂时锁定…' : '✨ 先点自己的小棋子，再点目标位置~'}</p>`;
    }
    const cells = []; for (let r = 0; r < 10; r++) for (let c = 0; c < 9; c++) {
      const idx = r * 9 + c; const value = board[idx] || 0; const selected = s.sel === idx;
      const isLastTo = idx === lastTo; const isLastFrom = idx === lastFr;
      const lastCls = isLastTo ? `is-last-to ${lastSide}` : isLastFrom ? `is-last-from ${lastSide}` : '';
      cells.push(`<button class="xq-cell ${selected ? 'is-selected' : ''} ${value ? 'has-piece' : ''} ${lastCls}" data-i="${idx}" ${locked ? 'disabled' : ''}>${icons.xiangqiPieceHtml(value)}</button>`);
    }
