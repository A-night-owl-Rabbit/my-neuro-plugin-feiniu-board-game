...
> [my-neuro(肥牛)](https://github.com/morettt/my-neuro) 社区插件发布仓库 · 当前版本：0.5.1
...
- 装上它，对肥牛说「来下盘五子棋」，它会打开一个棋局网页（默认 `http://127.0.0.1:22335`），你们就能在浏览器里对弈，肥牛还会边下边吐槽解说
- **棋力和心情挂钩**：本插件必须搭配 `mood-chat`（潮汐之音）插件使用——肥牛心情越好棋力越差（让着你），心情差时棋力反而变强
- 两个可选增强：① 填一个 DeepSeek API Key 让肥牛在棋局中实时解说；② 下载 pikafish 引擎让中国象棋变成职业级对局（都不填也能玩，只是少了解说、象棋用内置 AI）

## 环境要求

- my-neuro（live-2d 桌宠）近期版本
- **必装**：`mood-chat` 插件（心情系统是本插件棋力与局后反应的基础）
- 可选：DeepSeek API Key（过程解说用，[platform.deepseek.com](https://platform.deepseek.com/) 可申请）；pikafish 引擎（象棋进阶，见下文教程）
...
2. 把解压出的文件夹放入（确保 `index.js` 直接位于该目录下，不要多套一层）：
   ...
   `my-neuro/live-2d/plugins/community/feiniu-board-game/`

3. 重启 Live2D 桌宠，在 WebUI「插件管理」中启用本插件与 mood-chat

## 使用教程（照着说就行）

- 「来下盘五子棋」/「我们下象棋吧」→ 肥牛自动打开棋局网页
- 浏览器访问 `http://127.0.0.1:22335` 也能直接打开（关掉页面不会丢对局，重开接着下）
- 「这盘不算，重来」/「关闭棋局」→ 结束对局
- 想让肥牛亲自思考每一步（棋力弱但更像"本人在下"）：配置里把 `ai_move_mode` 改为 `llm`，详见下方「LLM 亲自下棋模式」
...
- **打开棋局说连不上页面？** 棋局网页由本地 HTTP 服务提供（默认 `http://127.0.0.1:22335`），确认桌宠在运行、端口没被占用；可在配置里换 Web UI 端口。
- **肥牛下棋太强/太弱？** 棋力默认跟随 mood-chat 心情（心情 ≥90 最弱、<60 最强）。想固定难度把 `chess_chat_difficulty_override` 设为 0~3，详见上文难度表。
- **下棋时肥牛不说话？** 过程解说需要填 `chess_chat_deepseek_api_key`（留空即自动关闭解说）；填了还不开，检查模型名是否为该平台真实存在的名字（DeepSeek 官方是 `deepseek-chat`）。
- **关掉浏览器对局还在吗？** 在。会话由本地服务保持，重新打开同一地址可继续这局。
- **"惩罚/局后注入"类配置能改吗？** 能。这些注入文案、惩罚项目名、强度、安全词都在插件配置里，可全部自定义；对局中说出安全词即停。
- **为什么必须装 mood-chat？** 心情分驱动棋力档位与局后心情变化，是本插件设计的核心依赖；不装则棋力联动与 `adjustMood` 无法工作。

## 故障排查

1. 棋局网页打不开：看终端日志里本地 HTTP 服务的启动与端口信息，确认端口与配置一致
2. 解说无声音：确认 `chess_chat_enabled` 为 true、Key 有效、TTS 正常；模型名错误会 404 静默降级（仅终端 warn）
3. 象棋没接上 pikafish：看日志 `pikafish 引擎已就绪` / `pikafish 引擎不可用`，按上文教程检查 `bin/` 目录或绝对路径
4. 还有问题请到 [本仓库 Issues](https://github.com/A-night-owl-Rabbit/my-neuro-plugin-feiniu-board-game/issues) 提问

## 更新与卸载

- **更新**：插件广场点更新，或手动覆盖 `community/feiniu-board-game/` 后重启；pikafish 二进制放在 `bin/` 里更新时注意别删
- **卸载**：WebUI「插件管理」停用并删除 `live-2d/plugins/community/feiniu-board-game/` 文件夹即可

## 版本

- **0.5.1**：浏览器网页版棋盘、LLM 亲自下棋模式、过程解说、pikafish 接入、alpha-beta 棋力提升（详见上文 v0.5 功能摘要）。
- 本教程最后更新：2026-09
